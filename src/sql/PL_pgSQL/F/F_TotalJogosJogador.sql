-- Aqui usa uma função de PSQL

CREATE OR REPLACE FUNCTION total_jogos_jogador(uid INTEGER) RETURNS INTEGER
AS
$$
DECLARE
...
BEGIN
	...
	select  COUNT(DISTINCT(game_ref)) from single_player_match where player_id = uid into total_jogos_single;
	
    SELECT COUNT(DISTINCT(pm.game_ref))
